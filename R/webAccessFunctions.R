...
.ftpFileInfo <- function(url, filename, tag, verbose=TRUE) {
  # warning("Attempting to get ftp file info for url(s):\n", url, immediate.=TRUE)
  .ftpFileInfoNew(url, filename, tag, verbose)
  # .ftpFileInfoOld(url, filename, tag, verbose)
}

.ftpFileInfoOld <- function(url, filename, tag, verbose=TRUE) {
  ## set the curl handle
  ...
  allurls <- lapply(url, function(ul){
    txt <- RCurl::getURL(ul, dirlistonly=TRUE, curl=curl)
    df2 <- strsplit(txt, "\r*\n")[[1]]
...
.ftpFileInfoNew <- function(url, filename, tag, verbose=TRUE) {
  firsturl <- ifelse(length(url)>1, url[1], url)
  ## make a list of filenames from each url
  allurls <- lapply(url, function(ul){
    # Establish the curl handle
    list_h <- curl::new_handle()
    curl::handle_setopt(list_h, dirlistonly=TRUE)
    # Open the connecting in "r" (read text) mode.
    con <- curl::curl(ul, "r")
    # Note, this will probably break now
    # warning("Connection established", immediate.=TRUE)

    txt <- read.table(con, stringsAsFactors=FALSE, fill=TRUE)
    # No need to perform platform specific string manipulation
    # warning("txt received", immediate.=TRUE)

    df2 <- txt[[9]]
    # warning("df2 created", immediate.=TRUE)

    df2 <- df2[grep(paste0(filename, "$"), df2)]
    # warning("df2): ", df2, immediate.=TRUE)


    # I don't understand the point of this lin
    # TODO: Ask Dan about this
    drop <- grepl("00-", df2)
    df2 <- df2[!drop]
    # warning("df2[!drop] : ", df2, immediate.=TRUE)
    base::close(con)

    paste0(ul, df2)
  })
  allurls <- unlist(allurls)
  warning("unlisted, allurls: ", allurls, immediate.=TRUE)
...
.fileExistsOnline  <- function(file) {
   sapply(file, function(z) {
       try(
           z <- RCurl::getBinaryURL(file, failonerror = TRUE)
       )
       if (length(z) > 1)
           TRUE
       else
...
## Replacement function for the following :
##  RCurl::getURL(ul], followlocation=TRUE, customrequest="LIST -R")
## The aim is to provide a single entry point for this block, so that we can
## replace 'RCurl' with 'curl'
.listRemoteFiles <- function(someUrl, filename, tag) {
  listing <- RCurl::getURL(url=someUrl, followlocation=TRUE, customrequest="LIST -R")
  # Return listing as rows
  return (strsplit(listing, "\r*\n")[[1]]);
}

.ftpDirectoryInfo <- function(someUrl){
  listing <- RCurl::getURL(url=someUrl, dirlistonly=TRUE, followlocation=TRUE)
  return (strsplit(listing, "\r*\n")[[1]]);
}
